...
export const gradeDraftFn = inngest.createFunction(
  ...
  async ({ event, step }: any) => {
    const { draft, type = 'collection', platform, brandVoice, seoRules, metafieldDefinitions, placement, products, brief, research, metafieldSamples } = event.data;
    return await step.run('grade-draft', async () => {
      return gradeDraft({ draft, type, platform, brandVoice, seoRules, metafieldDefinitions, placement, products, brief, research, metafieldSamples });
    });
  }
);

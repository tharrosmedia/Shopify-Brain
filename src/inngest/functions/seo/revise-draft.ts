...
export const reviseDraftFn = inngest.createFunction(
  ...
  async ({ event, step }: any) => {
    const { draft, feedback, type = 'collection', platform, brandVoice, seoRules, metafieldDefinitions, placement, products, brief, research } = event.data;
    return await step.run('revise-draft', async () => {
      return reviseDraft({ draft, feedback, type, platform, brandVoice, seoRules, metafieldDefinitions, placement, products, brief, research });
    });
  }
);

...
export const optimizeDraftFn = inngest.createFunction(
  ...
  async ({ event, step }: any) => {
    const { storeId, draft, type = 'collection', platform, brandVoice, metafieldDefinitions, placement } = event.data;
    return await step.run('optimize-draft', async () => {
      return optimizeDraft({ storeId, draft, type, platform, brandVoice, metafieldDefinitions, placement });
    });
  }
);

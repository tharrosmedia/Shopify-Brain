...
export const writeDraftFn = inngest.createFunction(
  ...
  async ({ event, step }: any) => {
    const { storeId, brief, type = 'collection', platform, brandVoice, metafieldDefinitions, placement } = event.data;
    return await step.run('write-draft', async () => {
      return writeDraft({ storeId, brief, type, platform, brandVoice, metafieldDefinitions, placement });
    });
  }
);

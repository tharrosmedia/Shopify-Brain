...
export const publishFn = inngest.createFunction(
  ...
  async ({ event, step }: any) => {
    const { storeId, draft, type = 'collection', platform, brandVoice, products, metafieldDefinitions } = event.data;
    return await step.run('publish-content', async () => {
      return publishContent({ storeId, draft, type, platform, brandVoice, products, metafieldDefinitions });
    });
  }
);

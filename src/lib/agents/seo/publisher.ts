...
import type * as Collections from '../../shopify/collections';
import type * as Pages from '../../shopify/pages';
import type * as Blogs from '../../shopify/blogs';
...
export async function publishContent({ storeId, draft, type = 'collection', platform, brandVoice, products = [], preloaded, metafieldDefinitions = [], mode = 'create', shopifyId }: { storeId: string; draft: any; type?: string; platform?: string; brandVoice?: any; products?: any[]; preloaded?: { domain?: string; accessToken?: string; config?: any }; metafieldDefinitions?: any[]; mode?: string; shopifyId?: string; [k: string]: any }) {
  ...
  if (type === 'page') {
    if (isImprove && useId) {
      const { updatePage } = await import('../../shopify/pages') as typeof Pages;
    ...
    if (isImprove && useId) {
      const { updateArticle } = await import('../../shopify/blogs') as typeof Blogs;
    ...
    if (isImprove && useId) {
      const { updateCollection } = await import('../../shopify/collections') as typeof Collections;

...
export async function writeDraft({ storeId, brief, type = 'collection', platform, brandVoice }: { storeId: string; brief: any; type?: string; platform?: string; brandVoice?: any }) {
  const allMsgs = buildWriterMessages({ brief, type, brandVoice, platform });

...
export async function saveDraft({ jobId, storeId, title, handle, bodyHtml, metaTitle, metaDescription, metafields, schemaJsonLd, rawResearch, evaluationScores }: { jobId?: string; storeId: string; title: string; handle: string; bodyHtml: string; metaTitle?: string; metaDescription?: string; metafields?: any; schemaJsonLd?: any; rawResearch?: any; evaluationScores?: any }) {
  const sql = neon(process.env.DATABASE_URL!);
  const result = await sql`INSERT INTO drafts (job_id, store_id, title, handle, body_html, meta_title, meta_description, metafields, schema_jsonld, raw_research, evaluation_scores) VALUES (${jobId || null}, ${storeId}, ${title}, ${handle}, ${bodyHtml}, ${metaTitle || null}, ${metaDescription || null}, ${metafields || null}, ${schemaJsonLd || null}, ${rawResearch || null}, ${evaluationScores || null}) RETURNING id, job_id as "jobId", store_id as "storeId", title, handle, body_html as "bodyHtml", meta_title as "metaTitle", meta_description as "metaDescription", metafields, schema_jsonld as "schemaJsonLd", raw_research as "rawResearch", evaluation_scores as "evaluationScores", created_at as "createdAt"`;
  return result[0];
}
